...
class SingleFlowGenPipeline(DiffusionPipeline):
    ...
    def _encode_prompt(
        ...
        # Use Compel if available for long prompt support
        if self.compel is not None:
            ...
            test_tokens = self.tokenizer(
                prompt_list[0] if prompt_list else "",
                return_tensors="pt"
            )
            is_long_prompt = test_tokens.input_ids.shape[-1] > self.tokenizer.model_max_length
            
            if is_long_prompt:
                print(f"[Compel] Long prompt detected ({test_tokens.input_ids.shape[-1]} tokens > {self.tokenizer.model_max_length}), using Compel for encoding...")
            
            # Use compel to encode
            text_embeddings = self.compel(prompt)
            text_embeddings = text_embeddings.to(device)
            
        else:
            # Original encoding logic (fallback)
        ...
        if do_classifier_free_guidance:
            ...
            # Use Compel for negative prompt if available
            if self.compel is not None:
                uncond_embeddings = self.compel(uncond_tokens)
    ...
    def decode_latents(self, latents):
        ...
        video = self.vae_flow.decode(latents).sample
        # video = []
        # for frame_idx in tqdm(range(latents.shape[0])):
        #     video.append(self.vae_flow.decode(latents[frame_idx:frame_idx+1]).sample)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred = self.unet(
                    latent_model_input, t, stride, encoder_hidden_states=text_embeddings
